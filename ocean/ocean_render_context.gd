class_name OceanRenderContext extends Object
## ============================================================================
## PORTADO de GodotOceanWaves — MIT, Copyright (c) 2024 Ethan Truong
## https://github.com/2Retr0/GodotOceanWaves · ver shaders/ocean_fft/LICENSE_*.txt
## Adaptado a ModuleSystem: renombrado, rutas de shader del proyecto y
## profundidad configurable (el suelo oceanico alimenta el espectro).
## ============================================================================
...
static func create(device : RenderingDevice=null) -> OceanRenderContext:
	var context := OceanRenderContext.new()
...
func create_pipeline(block_dimensions : Array, descriptor_sets : Array, shader : RID) -> Callable:
	...
	return func(context : OceanRenderContext, compute_list : int, push_constant : PackedByteArray=[], descriptor_set_overwrites:=[], block_dimensions_overwrite_buffer:=RID(), block_dimensions_overwrite_buffer_byte_offset:=0) -> void:
...
## Empaqueta los datos como push constant, con el tamaño EXACTO que declara el
## shader (4 bytes por valor).
## CAMBIO vs. el original: NO se rellena a múltiplos de 16. Godot 4.3 lo aceptaba;
## Godot 4.7 VALIDA el tamaño y rechaza el dispatch entero:
##   "This compute pipeline requires (52) bytes of push constant data, supplied: (64)"
##   "The shader ... requires a push constant to be set before drawing, but it's not present."
## Con el relleno, NINGÚN dispatch corría y las texturas quedaban en cero (mar
## plano). Medido en Godot 4.7-stable.
